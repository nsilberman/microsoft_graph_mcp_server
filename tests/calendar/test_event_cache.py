...
from microsoft_graph_mcp_server.utils import DateHandler
...
from microsoft_graph_mcp_server.cache.event_cache import EventBrowsingCache
...
async def test_timezone_conversion():
    ...
    converted_time = date_handler.convert_utc_to_user_timezone(
...
async def test_event_cache_timezone_conversion():
    ...
    mock_event = {
        ...
        "responseStatus": {"response": "accepted", "time": "2026-01-04T10:00:00Z"},
        "attendees": [
            {
    ...
    start_converted = date_handler.convert_utc_to_user_timezone(
        start_datetime, user_timezone
    )
    end_converted = date_handler.convert_utc_to_user_timezone(
        end_datetime, user_timezone
    )
    ...
    assert response_time_converted is not None, (
        "Response status time should be converted"
    )
    assert len(mock_event["attendees"]) == 2, "Attendees count should be 2"
    assert mock_event.get("recurrence") is not None, (
        "Event should have recurrence property"
    )

    # Verify attendees have proper data
    attendee1 = mock_event["attendees"][0]
    assert attendee1["emailAddress"]["name"] == "John Doe", (
        "First attendee name should be John Doe"
    )
    assert attendee1["emailAddress"]["address"] == "john@example.com", (
        "First attendee email should be john@example.com"
    )
...
async def test_recurring_event_detection():
    ...
    is_recurring1 = event_with_recurrence.get(
        "recurrence"
    ) is not None or event_with_recurrence.get("type") in ["occurrence", "seriesMaster"]
    assert is_recurring1 is True, (
        "Event with recurrence property should be marked as recurring"
    )
    ...
    is_recurring2 = event_occurrence.get(
        "recurrence"
    ) is not None or event_occurrence.get("type") in ["occurrence", "seriesMaster"]
    assert is_recurring2 is True, (
        "Event with type='occurrence' should be marked as recurring"
    )
    ...
    is_recurring3 = event_series_master.get(
        "recurrence"
    ) is not None or event_series_master.get("type") in ["occurrence", "seriesMaster"]
    assert is_recurring3 is True, (
        "Event with type='seriesMaster' should be marked as recurring"
    )
    ...
    is_recurring4 = event_single.get("recurrence") is not None or event_single.get(
        "type"
    ) in ["occurrence", "seriesMaster"]
    assert is_recurring4 is False, (
        "Event without recurrence should not be marked as recurring"
    )
...
async def test_attendees_list_formatting():
    ...
    event_data = {
        ...
        "attendees_list": [
            ...
            {
                "emailAddress": {"name": "", "address": "charlie@example.com"},
                "status": {"response": "tentativelyAccepted"},
            },
        ],
    ...
    assert "Alice Johnson (alice@example.com)" in attendees_display, (
        "First attendee should be formatted correctly"
    )
    assert "Bob Smith (bob@example.com)" in attendees_display, (
        "Second attendee should be formatted correctly"
    )
    assert "charlie@example.com" in attendees_display, (
        "Third attendee (no name) should show only email"
    )
...
async def test_response_status_time_conversion():
    ...
    assert event_data["responseStatus"]["response"] == "accepted", (
...
async def test_event_cache_persistence():
    ...
    await cache.update_browse_state(
        start_date="2026-01-01", end_date="2026-01-31", total_count=2, metadata=events
    )
    ...
    assert len(loaded_events) == len(events), (
        "Loaded events count should match saved events count"
    )
    ...
    for saved_event in events:
        ...
        assert saved_event["subject"] == loaded_event["subject"], (
            f"Event subject should match"
        )
        assert saved_event["recurrence"] == loaded_event["recurrence"], (
            f"Event recurrence flag should match"
        )
...
async def test_format_user_timezone_datetime():
    ...
    assert "04:30 PM" in formatted or "16:30" in formatted, (
        f"Expected 4:30 PM in formatted time, got: {formatted}"
    )
    assert "Mon 01/05/2026" in formatted, (
        f"Expected date in formatted time, got: {formatted}"
    )
    ...
    assert "10:00 AM" in formatted2, (
        f"Expected 10:00 AM in formatted time, got: {formatted2}"
    )
    assert "Tue 01/06/2026" in formatted2, (
        f"Expected date in formatted time, got: {formatted2}"
    )
