...
class EmailBrowsingCache:
    ...
    def _create_new_cache(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                "user_id": None,
                "expires_at": (now + timedelta(hours=self.CACHE_EXPIRY_HOURS)).isoformat() + "Z"
            }
        }
    ...
    def _is_cache_valid(self, cache: Dict[str, Any]) -> bool:
        ...
        try:
            last_updated = datetime.fromisoformat(cache["last_updated"].replace("Z", ""))
    ...
    def _is_cache_expired(self) -> bool:
        ...
        try:
            expires_at = datetime.fromisoformat(self.cache["metadata"]["expires_at"].replace("Z", ""))
    ...
    async def update_list_state(self, folder: Optional[str] = None, top: Optional[int] = None, 
                          ...
                          total_count: Optional[int] = None, metadata: Optional[List[Dict[str, Any]]] = None):
    ...
    def get_cached_emails(self) -> List[Dict[str, Any]]:
        ...
        return sorted(metadata, key=lambda x: x.get("receivedDateTimeOriginal", ""), reverse=True)
    
    def should_refresh_total_count(self) -> bool:
        """Check if total_count needs to be refreshed."""
        state = self.cache["list_state"] if self.cache["mode"] == "list" else self.cache["search_state"]
